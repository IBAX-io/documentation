...
- *Object* - アクションリクエストオブジェクトを取得します。
    ...
    - **lang** - *String* - [Omitempty](#omitempty) - このフィールドは、マルチリンガルリソースコードまたはローカライゼーションを指定します。例：*en, ja*。指定されたマルチリンガルリソースが見つからない場合、例えば *en-US* の場合は、マルチリンガルリソースグループで検索します。**デフォルト**: **en**。
