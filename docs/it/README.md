---
home: true
heroImage: /ibax.png
heroText: IBAX
tagline: Una rete infrastrutturale cross-chain commerciale decentralizzata
actionText: Avvio veloce →
actionLink: /it/concepts/about-the-platform.html
features:
- title: Una grande riduzione del costo dell'applicazione blockchain
- title: Un ecosistema fisico costruito in modo indipendente
- title: Implementazione rapida di server distribuiti, ambienti di sviluppo e applicazioni
- title: Un DApp Store diversificato e funzionalità BaaS
- title: Risposta effettiva del servizio in pochi secondi
- title: Interazione dati cross-chain e cross-system
- title: Una nuova rete infrastrutturale che integra catene pubbliche, alleanze e rivali
- title: Un meccanismo di consenso personalizzabile
- title: Separazione di front-end e back-end per soddisfare le diverse esigenze di crittografia

footer: © Copyright 2019 - 2021, IBAX Updated Date 03/11/2021 | Built with VuePress
---


# IBAX Documentazione


## Concetto

* [IBAX Overview](concepts/about-the-platform.md)
* [The IBAX Network](concepts/blockchain-layers.md)
* [Proof-of-Authority Consensus](concepts/consensus.md)
* [Terms and Definitions](concepts/thesaurus.md)
* [FAQ](concepts/faq.md)

## Tutorial

* [Tutorial for application development](tutorials/app_tutorial.md)
* [Development Tutorial](tutorials/tutorial.md)

## Guida

* [Smart Contracts](topics/script.md)
* [Template Language](topics/templates2.md)
* [Compiler and Virtual Machine](topics/vm.md)
* [Daemon](topics/daemons.md)

## Riferimento

* [RESTful API](reference/api2.md)
* [Platform Parameters](reference/platform-parameters.md)
* [Server Configuration File](reference/backend-config.md)
* [Synchronized Monitoring Tool](reference/desync_monitor.md)
* [JSON-RPC Application Programming Interface](reference/json-rpc.md)

## Distribuzione

* [Deployment of A IBAX Network](howtos/deployment.md)

