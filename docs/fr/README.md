---
home: true
heroImage: /ibax.png
heroText: IBAX
tagline: Un réseau d'infrastructures commerciales inter-chaînes décentralisées
actionText: Démarrage rapide →
actionLink: /fr/concepts/about-the-platform.html
features:
- title: Une grande réduction du coût de l'application blockchain
- title: Un écosystème physique construit de manière indépendante
- title: Déploiement rapide de serveurs distribués, d'environnements de développement et d'applications
- title: Un DApp Store diversifié et des fonctionnalités BaaS
- title: Réponse réelle du service en quelques secondes
- title: Interaction des données inter-chaînes et inter-systèmes
- title: Un nouveau réseau d'infrastructures intégrant des chaînes publiques, alliances et rives
- title: Un mécanisme de consensus personnalisable
- title: Séparation du front-end et du back-end pour répondre aux divers besoins de cryptage

footer: © Copyright 2019 - 2021, IBAX Updated Date 03/11/2021 | Built with VuePress
---


# IBAX Documentation


## Concept

* [IBAX Overview](concepts/about-the-platform.md)
* [The IBAX Network](concepts/blockchain-layers.md)
* [Proof-of-Authority Consensus](concepts/consensus.md)
* [Terms and Definitions](concepts/thesaurus.md)
* [FAQ](concepts/faq.md)

## Didacticiel

* [Tutorial for application development](tutorials/app_tutorial.md)

## Guider

* [Smart Contracts](topics/script.md)
* [Template Language](topics/templates2.md)
* [Compiler and Virtual Machine](topics/vm.md)
* [Daemon](topics/daemons.md)

## Référence

* [RESTful API](reference/api2.md)
* [Platform Parameters](reference/platform-parameters.md)
* [Server Configuration File](reference/backend-config.md)
* [Synchronized Monitoring Tool](reference/desync_monitor.md)

## Déploiement

* [Deployment of A IBAX Network](howtos/deployment.md)

